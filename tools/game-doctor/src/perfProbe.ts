...
export function buildStressScenario(Sim: SimCtor = GameSimulation as SimCtor): GameSimulation {
...
export function workCount(ticks: number = DEFAULT_TICKS, Sim: SimCtor = GameSimulation as SimCtor): WorkCountResult {
  const sim = buildStressScenario(Sim);

  let visits = 0;
  const grid = sim.spatialGrid as unknown as {
    forEachInRadius: (x: number, y: number, r: number, cb: (c: unknown) => void) => void;
  };
 ...
 */
export function compareInterleaved(
  builds: { label: string; Sim: SimCtor }[],
  ticks: number = DEFAULT_TICKS,
): InterleavedResult[] {
  const sims = builds.map((b) => ({ label: b.label, sim: buildStressScenario(b.Sim), samples: [] as number[] }));
  for (let t = 0; t < ticks; t++) {
