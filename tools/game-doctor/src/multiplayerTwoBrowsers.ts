...
function startGameServer(): ChildProcess {
  ...
  child.stdout?.on('data', (chunk: Buffer) => {
    ...
    if (line.includes('running on') || line.includes('DESYNC')) console.log(`   [server] ${line.slice(0, 200)}`);
  });
...
async function joinLobbyViaUI(page: Page, playerName: string): Promise<void> {
  ...
  if (isDeferred > 0) throw new Error('MULTIPLAYER menu item is still marked as deferred (dead button)');
  ...
  await page.waitForFunction(
    () => document.querySelectorAll('.ra4-multiplayer-lobby .ra4-roster-row').length > 0,
    undefined,
    { timeout: 20_000 }
  );
...
export async function runMultiplayerTwoBrowsers(): Promise<boolean> {
  ...
  try {
    ...
    for (const [label, page] of [['A', pageA], ['B', pageB]] as const) {
      page.on('console', (msg) => {
        if (msg.type() === 'error') consoleErrors.push(`[${label}] ${msg.text().slice(0, 200)}`);
      });
    }
    ...
    if (!started) throw new Error('Neither client presented an enabled "НАЧАТЬ БИТВУ" (host) button');
    ...
    for (const [label, page] of [['A', pageA], ['B', pageB]] as const) {
      ...
      await page.waitForFunction(
        () => {
          const probe = (window as any).__RA4_GAME_DOCTOR__;
          return probe && typeof probe.getNetworkStatus === 'function' && probe.getNetworkStatus().tick > 0;
        },
        undefined,
        { timeout: 60_000 }
      );
      const probe = await page.evaluate(() => (window as any).__RA4_GAME_DOCTOR__.getNetworkStatus() as ClientProbe);
    ...
    const probeA = await pageA.evaluate(() => (window as any).__RA4_GAME_DOCTOR__.getNetworkStatus() as ClientProbe);
    const probeB = await pageB.evaluate(() => (window as any).__RA4_GAME_DOCTOR__.getNetworkStatus() as ClientProbe);
    const snapA = await pageA.evaluate(() => { const s = (window as any).__RA4_GAME_DOCTOR__.getSnapshot(); return s ? { tick: s.tick, checksum: s.checksum, entities: s.entities.length } : null; });
    const snapB = await pageB.evaluate(() => { const s = (window as any).__RA4_GAME_DOCTOR__.getSnapshot(); return s ? { tick: s.tick, checksum: s.checksum, entities: s.entities.length } : null; });
    ...
    const historyA = await pageA.evaluate(() => (window as any).__RA4_GAME_DOCTOR__.getChecksumHistory() as Record<string, number>);
    const historyB = await pageB.evaluate(() => (window as any).__RA4_GAME_DOCTOR__.getChecksumHistory() as Record<string, number>);
    ...
    if (probeA.tick < 60) failures.push(`client A applied only ${probeA.tick} authoritative ticks`);
    if (probeB.tick < 60) failures.push(`client B applied only ${probeB.tick} authoritative ticks`);
    if (probeA.desync) failures.push('client A reported a desync against the server');
    if (probeB.desync) failures.push('client B reported a desync against the server');
    if (!snapA || !snapB) failures.push('a client produced no world snapshot');
    ...
    console.error('❌ [MP-E2E] Harness error:', (error as Error).message);
    return false;
  } finally {
    ...
    if (!passed) console.log('ℹ️  [MP-E2E] See logs above for the first failing step.');
  }
}
