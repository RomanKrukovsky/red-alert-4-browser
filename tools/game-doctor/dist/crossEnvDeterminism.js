...
export async function runCrossEnvDeterminism() {
    ...
    let browserChecksum = null;
    const browser = await chromium.launch({ headless: true });
    try {
        const page = await browser.newPage();
        await page.goto('http://localhost:5173/#/splash', { waitUntil: 'domcontentloaded' });
        await page.waitForFunction(() => typeof window.__RA4_DETERMINISM_PROBE__ === 'function', undefined, { timeout: 30_000 });
        console.log(`🔬 [CrossEnv] Browser Worker simulation: seed=${SEED}, ticks=${TICKS}...`);
        browserChecksum = await page.evaluate(([seed, ticks]) => window.__RA4_DETERMINISM_PROBE__(seed, ticks), [SEED, TICKS]);
