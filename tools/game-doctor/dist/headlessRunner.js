...
export class HeadlessTestRunner {
    ...
    async testHarvesterEconomyCycle() {
    ...
    async testArmyProductionAndCombat() {
        const startTime = Date.now();
        try {
    ...
    async testHQDestructionAndVictoryDefeat() {
        const startTime = Date.now();
        try {
    ...
    async testLossOfHarvesterAndPowerPlant() {
        const startTime = Date.now();
        try {
    ...
    async testDeterminismAndChecksum() {
        const startTime = Date.now();
        try {
            const seed = 42;
            const playerConfigs = [
    ...
    async testThirtyMinuteSimulation() {
        const startTime = Date.now();
        try {
            ...
            for (let t = 0; t < 3000; t++) {
                sim.step();
                if (sim.matchState === MatchState.FINISHED)
                    break;
            }
            return {
