...
class NodeHeap {
  ...
  public push(cellIndex: number, f: number, h: number): void {
    const i = this.size++;
    this.f[i] = f; this.h[i] = h; this.seq[i] = this.counter++; this.idx[i] = cellIndex;
    this.bubbleUp(i);
  }
  ...
  private bubbleUp(i: number): void {
    while (i > 0) {
      const parent = (i - 1) >> 1;
      if (this.less(i, parent)) { this.swap(i, parent); i = parent; } else break;
    }
  }
...
export class NavigationService {
  ...
  private version = 0;
  private flowCache = new Map<number, FlowField>();
  private static readonly FLOW_CACHE_MAX = 64;
  private static readonly UNREACHABLE = 0xffff;
    ...
    if (passabilityGrid) {
      for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
          if (passabilityGrid[y]?.[x]) this.grid[y * width + x] = 1;
        }
      }
    }
  }
   ...
   */
  public registerObstacle(gridX: number, gridY: number, gridWidth: number, gridHeight: number): void {
    this.stampObstacle(gridX, gridY, gridWidth, gridHeight, +1);
  }

  public unregisterObstacle(gridX: number, gridY: number, gridWidth: number, gridHeight: number): void {
    this.stampObstacle(gridX, gridY, gridWidth, gridHeight, -1);
  }

  private stampObstacle(gridX: number, gridY: number, gridWidth: number, gridHeight: number, delta: number): void {
  ...
  public findPath(startX: number, startY: number, endX: number, endY: number): ScaledVector2[] {
    ...
    if (!this.isWalkable(egx, egy)) {
      ...
      if (!near) return [{ x: endX, y: endY }];
      egx = near.x; egy = near.y;
    }
    ...
    while (heap.length > 0) {
      ...
      for (let d = 0; d < DIRS.length; d++) {
        ...
        if (DIRS[d][2] === 14 && (!this.isWalkable(cx + DIRS[d][0], cy) || !this.isWalkable(cx, cy + DIRS[d][1]))) continue;
        const ni = ny * w + nx;
    ...
    for (let i = goalIdx; i !== -1; i = cameFrom[i]) tilePath.push(i);
    tilePath.reverse();
   ...
   */
  public getFlowField(goalWorldX: number, goalWorldY: number): FlowField | null {
    ...
    if (this.flowCache.size >= NavigationService.FLOW_CACHE_MAX) {
      ...
      if (oldest !== undefined) this.flowCache.delete(oldest);
    }
    this.flowCache.set(key, field);
    return field;
  }

  private buildFlowField(goalGx: number, goalGy: number): FlowField {
    const w = this.width;
    const cellCount = w * this.height;
    ...
    for (let i = 0; i < cellCount; i++) {
      if (cost[i] === NavigationService.UNREACHABLE || i === goalIdx) continue;
      const cx = i % w;
      const cy = (i - cx) / w;
      let best = -1;
      let bestCost = cost[i];
   ...
   */
  public sampleFlow(field: FlowField, worldX: number, worldY: number): { dx: number; dy: number } | null {
  ...
  public isReachable(field: FlowField, worldX: number, worldY: number): boolean {
    const gx = this.clampX(Math.floor(worldX / TILE));
    const gy = this.clampY(Math.floor(worldY / TILE));
  ...
  public findNearestWalkableTile(gx: number, gy: number): { x: number; y: number } | null {
    for (let r = 1; r <= 8; r++) {
  ...
  private clampX(v: number): number { return v < 0 ? 0 : v >= this.width ? this.width - 1 : v; }
  private clampY(v: number): number { return v < 0 ? 0 : v >= this.height ? this.height - 1 : v; }
}
