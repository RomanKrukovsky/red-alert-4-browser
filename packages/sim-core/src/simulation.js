...
export class GameSimulation {
            ...
            case CommandType.MOVE: {
                for (const id of cmd.entityIds) {
                    ...
                    if (e && e.playerIndex === cmd.playerIndex && !e.isBuilding) {
                        e.targetX = cmd.targetX;
                        e.targetY = cmd.targetY;
    ...
    step() {
        ...
        // 3. Movement Logic
        for (const e of this.entities.values()) {
            if (!e.isBuilding && e.targetX !== undefined && e.targetY !== undefined) {
                const distSq = fixedDistanceSq(e.x, e.y, e.targetX, e.targetY);
                ...
                if (distSq <= stepDistSq) {
                    ...
                    const dx = (e.targetX - e.x) / dist;
                    const dy = (e.targetY - e.y) / dist;
                ...
                if (e.playerIndex === pIdx) {
                    if (e.isBuilding) {
                        ...
                        if (spec) {
                            ...
                            if (spec.category === BuildingCategory.HQ)
                                hasHQ = true;
                        }
                    }
                    else {
