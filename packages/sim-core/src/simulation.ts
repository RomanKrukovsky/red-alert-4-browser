import { ArmorType, assertNever, BuildingCategory, CommandType, FactionId, MatchState, OrderMode, PassabilityType, PlayerCommand, PlayerEconomyState, PlayerType, TechTier, UnitCategory, UnitStance, VeterancyRank, WorldSnapshot } from '@ra4/shared-types';
...
export class GameSimulation {
  ...
  constructor(seed: number = 1337, mapWidth?: number, mapHeight?: number, mapId?: string) {
    this.seed = seed;
    const map = mapId
    ...
    switch (cmd.type) {
      ...
      case CommandType.GUARD: {
        ...
        for (const e of units) {
          ...
          if (guarded && this.isEnemy(e, guarded)) continue;
      ...
      case CommandType.SET_STANCE: {
        for (const id of cmd.entityIds) {
          ...
          if (e && e.playerIndex === cmd.playerIndex) {
            ...
            if (cmd.stance === UnitStance.HOLD_FIRE) e.targetEntityId = undefined;
          }
        }
        break;
      }
   ...
   */
  private clearOrders(e: SimEntity): void {
    e.targetX = undefined;
    e.targetY = undefined;
    e.targetEntityId = undefined;
    e.waypoints = undefined;
  ...
  private setMoveDestination(e: SimEntity, x: number, y: number): void {
    e.targetX = x;
    e.targetY = y;
    e.flowGoalX = undefined;
    e.flowGoalY = undefined;
   ...
   */
  private static readonly GUARD_LEASH = 6000; // 6 tiles (scaled ints)
   ...
   */
  private updateStandingOrders(): void {
    ...
    for (const e of this.entities.values()) {
      ...
      if (e.orderMode === OrderMode.NONE || e.orderMode === OrderMode.ATTACK_MOVE) continue;
      if (e.isBuilding || e.isDisabled || e.hp <= 0) continue;
      ...
      switch (e.orderMode) {
        ...
        case OrderMode.PATROL: {
          ...
          if (e.targetEntityId !== undefined && this.entities.has(e.targetEntityId)) break;
        ...
        case OrderMode.GUARD: {
          ...
          if (e.postX === undefined || e.postY === undefined) break;
    ...
    for (const e of this.entities.values()) {
      ...
      if (e.weaponId && e.attackCooldown === 0 && !e.isDisabled) {
        ...
        if (!target) {
          ...
          if (e.stance === UnitStance.HOLD_FIRE) continue;
          ...
          const scan = (range: number): SimEntity | undefined => {
            let minDistSq = range * range;
            let found: SimEntity | undefined;
  ...
  public calculateChecksum(): number {
    ...
    for (const e of this.entities.values()) {
      ...
      mix(e.patrolIndex ?? -1);
      mix(e.postX ?? -1); mix(e.postY ?? -1);
      mix(e.guardEntityId ?? -1);
      if (e.patrolRoute) for (const leg of e.patrolRoute) { mix(leg.x); mix(leg.y); }
      for (const item of e.productionQueue) {
