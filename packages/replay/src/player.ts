...
import { decodeReplay, ReplayDataV2, ReplayFormatError } from './format.js';
...
export class ReplayPlayerV2 {
  public readonly data: ReplayDataV2;
  public sim: GameSimulation;
  public currentTick = 0;
  private frameIndex = 0;
  private checkpointIndex = 0;
  private divergenceTick: number | null = null;
  private checkpointsChecked = 0;
  private lastDivergence: { expected: number; actual: number } | null = null;
  ...
  public step(): boolean {
    if (this.currentTick >= this.data.header.durationTicks) return false;
  ...
  public previewAt(tick: number): WorldSnapshot | null {
    let best: WorldSnapshot | null = null;
    for (const kf of this.data.keyframes) {
      if (kf.tick <= tick) best = kf.snapshot;
      else break;
    }
    return best;
  }
  ...
  public seekToTick(targetTick: number): void {
  ...
  public verify(): ReplayVerification {
    this.seekToTick(0);
