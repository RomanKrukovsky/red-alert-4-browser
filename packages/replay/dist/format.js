 ...
 */
export const REPLAY_MAGIC = [0x52, 0x41, 0x34, 0x52]; // 'RA4R'
...
export function decodeReplay(bytes) {
    ...
    for (const expected of REPLAY_MAGIC) {
        if (r.u8() !== expected)
            throw new ReplayFormatError('BAD_MAGIC', 'not an RA4R replay file');
    }
    const version = r.u16();
    if (version !== REPLAY_FORMAT_VERSION) {
        throw new ReplayFormatError('VERSION_UNSUPPORTED', `replay format v${version} is not supported by this build (supported: v${REPLAY_FORMAT_VERSION}). ` +
    ...
    for (let i = 0; i < frameCount; i++) {
        const tick = r.u32();
        if (tick <= prevTick)
            throw new ReplayFormatError('FRAME_ORDER', `frame ticks not ascending at index ${i}`);
        prevTick = tick;
    ...
    for (let i = 0; i < kfCount; i++) {
        ...
        for (let b = 0; b < len; b++)
            json[b] = r.u8();
        keyframes.push({ tick, snapshot: JSON.parse(textDecoder.decode(json)) });
    }
