...
export function validatePlayerCommand(cmd, sim, playerIndex) {
    if (cmd.playerIndex !== playerIndex)
        return fail('Command playerIndex spoofing attempt');
    const p = sim.players[playerIndex];
    if (!p)
        return fail('Player index out of bounds');
    if (sim.surrenderedPlayers.has(playerIndex))
        return fail('Player has surrendered');
    ...
    const ownedSpecIds = () => {
        ...
        for (const entity of sim.entities.values()) {
            if (entity.playerIndex === playerIndex)
                set.add(entity.specId);
        }
        return set;
    };
    const validateOwnedMobileEntities = () => {
        for (const id of cmd.entityIds) {
            const entity = sim.entities.get(id);
            if (!entity)
                return fail(`Entity ${id} does not exist`);
            if (entity.playerIndex !== playerIndex)
                return fail(`Entity ${id} belongs to player ${entity.playerIndex}`);
        }
        return ok;
    };
    const validateTargetInBounds = (x, y) => {
    ...
    switch (cmd.type) {
        ...
        case CommandType.ATTACK: {
            ...
            if (!target)
                return fail(`Attack target ${cmd.targetEntityId} does not exist`);
            if (target.playerIndex === playerIndex)
                return fail('Cannot attack own entity');
            return ok;
        }
        ...
        case CommandType.BUILD_STRUCTURE: {
            ...
            if (!spec)
                return fail(`Unknown structure ${cmd.structureId}`);
            ...
            for (const prereq of spec.prerequisites) {
                if (!owned.has(prereq))
                    return fail(`Missing prerequisite ${prereq} for ${cmd.structureId}`);
            }
            return ok;
        }
        case CommandType.PRODUCE_UNIT: {
            ...
            if (!producer)
                return fail('Producer entity does not exist');
            if (producer.playerIndex !== playerIndex)
                return fail('Producer ownership mismatch');
            if (!producer.isBuilding)
                return fail('Producer is not a building');
            const spec = UNIT_BY_ID.get(cmd.unitId);
            if (!spec)
                return fail(`Unknown unit ${cmd.unitId}`);
            if (spec.factionId !== sim.playerFactions[playerIndex]) {
            ...
            for (const prereq of spec.prerequisites) {
                if (!owned.has(prereq))
                    return fail(`Missing prerequisite ${prereq} for ${cmd.unitId}`);
            }
            if (producer.productionQueue.length >= 10)
                return fail('Production queue is full');
            return ok;
        }
        ...
        case CommandType.REPAIR_STRUCTURE: {
            ...
            if (!structure)
                return fail('Structure does not exist');
            if (structure.playerIndex !== playerIndex)
                return fail('Structure ownership mismatch');
            if (!structure.isBuilding)
                return fail('Entity is not a structure');
            return ok;
        }
        ...
        case CommandType.CAPTURE_BUILDING: {
            ...
            if (!target)
                return fail('Capture target does not exist');
            if (!target.isBuilding)
                return fail('Capture target is not a building');
            if (target.playerIndex === playerIndex)
                return fail('Cannot capture own building');
            return ok;
        }
