import { CommandType, UnitStance } from '@ra4/shared-types';
...
const TAG_TO_TYPE = Object.fromEntries(Object.entries(TYPE_TO_TAG).map(([k, v]) => [v, k]));
export const MAX_COMMANDS_PER_FRAME = 256;
export const MAX_ENTITIES_PER_COMMAND = 200;
export function encodeCommand(w, cmd) {
    const tag = TYPE_TO_TAG[cmd.type];
    if (tag === undefined)
        throw new WireError('UNKNOWN_COMMAND', `no codec for command type ${cmd.type}`);
    w.u8(tag);
    w.u8(cmd.playerIndex);
    w.u32(cmd.tick);
    const ids = cmd.entityIds ?? [];
    if (ids.length > MAX_ENTITIES_PER_COMMAND)
        throw new WireError('TOO_MANY_ENTITIES', `${ids.length} > ${MAX_ENTITIES_PER_COMMAND}`);
    w.u16(ids.length);
    for (const id of ids)
        w.i32(id);
    switch (cmd.type) {
        ...
        case CommandType.BUILD_STRUCTURE:
            w.str(cmd.structureId);
            w.i32(cmd.gridX);
            w.i32(cmd.gridY);
            break;
        case CommandType.PRODUCE_UNIT:
            w.i32(cmd.producerEntityId);
            w.str(cmd.unitId);
            break;
        case CommandType.CANCEL_PRODUCTION:
            w.i32(cmd.producerEntityId);
            w.i32(cmd.queueIndex);
            break;
...
export function decodeCommand(r) {
    const tag = r.u8();
    const type = TAG_TO_TYPE[tag];
    if (!type)
        throw new WireError('UNKNOWN_COMMAND_TAG', `tag ${tag}`);
    const playerIndex = r.u8();
    const tick = r.u32();
    const idCount = r.u16();
    if (idCount > MAX_ENTITIES_PER_COMMAND)
        throw new WireError('TOO_MANY_ENTITIES', `${idCount}`);
    const entityIds = [];
    for (let i = 0; i < idCount; i++)
        entityIds.push(r.i32());
    const base = { playerIndex, tick, entityIds };
    switch (type) {
        case CommandType.MOVE:
        case CommandType.ATTACK_MOVE:
            return { ...base, type, targetX: r.i32(), targetY: r.i32() };
        case CommandType.PATROL: {
            const targetX = r.i32();
            const targetY = r.i32();
            const append = r.u8() === 1;
            return { ...base, type, targetX, targetY, ...(append ? { append: true } : {}) };
        }
        case CommandType.ATTACK:
            return { ...base, type, targetEntityId: r.i32() };
        case CommandType.GUARD: {
        ...
        case CommandType.SURRENDER:
            return { ...base, type };
        case CommandType.BUILD_STRUCTURE:
            return { ...base, type, structureId: r.str(), gridX: r.i32(), gridY: r.i32() };
        case CommandType.PRODUCE_UNIT:
            return { ...base, type, producerEntityId: r.i32(), unitId: r.str() };
        case CommandType.CANCEL_PRODUCTION:
            return { ...base, type, producerEntityId: r.i32(), queueIndex: r.i32() };
        case CommandType.SELL_STRUCTURE:
            return { ...base, type, structureEntityId: r.i32() };
        case CommandType.REPAIR_STRUCTURE:
            return { ...base, type, structureEntityId: r.i32() };
        case CommandType.USE_ABILITY: {
        ...
        case CommandType.CAPTURE_BUILDING:
            return { ...base, type, targetStructureId: r.i32() };
        case CommandType.DEPOSIT_ORE:
            return { ...base, type, refineryEntityId: r.i32() };
        case CommandType.GATHER: {
            const resourceNodeId = r.str();
            return { ...base, type, ...(resourceNodeId ? { resourceNodeId } : {}) };
        }
    }
}
export function encodeCommandList(commands) {
...
export function decodeCommandList(payload) {
    const r = new BinReader(payload);
    const count = r.u16();
    if (count > MAX_COMMANDS_PER_FRAME)
        throw new WireError('TOO_MANY_COMMANDS', `${count}`);
    const out = [];
    for (let i = 0; i < count; i++)
        out.push(decodeCommand(r));
    return out;
}
export function encodeTickFrame(frame) {
...
export function decodeTickFrame(payload) {
    const r = new BinReader(payload);
    const tick = r.u32();
    const count = r.u16();
    if (count > MAX_COMMANDS_PER_FRAME)
    ...
    const commands = [];
    for (let i = 0; i < count; i++)
        commands.push(decodeCommand(r));
    return { tick, commands };
}
export function encodeChecksum(p) {
    const w = new BinWriter(8);
    w.u32(p.tick);
    w.u32(p.checksum);
    return w.finish();
}
export function decodeChecksum(payload) {
    const r = new BinReader(payload);
    return { tick: r.u32(), checksum: r.u32() };
}
//# sourceMappingURL=commandCodec.js.map