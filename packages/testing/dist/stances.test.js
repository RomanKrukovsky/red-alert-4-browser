...
import { CommandType, FactionId, OrderMode, PlayerType, UnitStance } from '@ra4/shared-types';
let failures = 0;
...
// ── PATROL: cycles between waypoints ─────────────────────────────────────
{
    ...
    for (let t = 0; t < 900; t++) {
        ...
        if (s.x >= 31000)
            reachedFar = true;
        if (reachedFar)
            minXAfterOutbound = Math.min(minXAfterOutbound, s.x);
    }
...
// ── Determinism: same orders + seed ⇒ identical checksum ─────────────────
{
    ...
    check('Determinism: identical checksum across runs with standing orders', s1.calculateChecksum() === s2.calculateChecksum(), `${s1.calculateChecksum()} vs ${s2.calculateChecksum()}`);
...
//# sourceMappingURL=stances.test.js.map