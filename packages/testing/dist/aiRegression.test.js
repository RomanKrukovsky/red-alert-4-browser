...
console.log('Running AI correctness regression tests...');
{
    ...
    assert(aggressive.blackboard.targetHarvesterCount < economic.blackboard.targetHarvesterCount, 'AI personalities must change economic targets');
...
}
{
    ...
    assert(!commands.some(command => command.type === CommandType.ATTACK_MOVE), 'AI must not attack an unexplored fallback coordinate without intelligence');
}
{
    const sim = createDuel();
...
}
{
    ...
    for (let tick = 0; tick < 5_000; tick++)
        sim.step();
    const foreignUnits = Array.from(sim.entities.values()).filter(entity => entity.playerIndex === 1 && !entity.isBuilding && entity.factionId !== FactionId.ALLIANCE);
    assert(foreignUnits.length === 0, 'Alliance AI must never produce units from another faction');
    assert(Array.from(sim.entities.values()).some(entity => entity.playerIndex === 1 && entity.specId === 'AL_BulwarkMBT'), 'Alliance AI must unlock and produce its main battle tank after building the Intel Center');
}
console.log('SUCCESS! AI correctness regression tests passed.');
//# sourceMappingURL=aiRegression.test.js.map