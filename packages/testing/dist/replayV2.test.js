...
// ── Headless verification: re-simulation matches all checkpoints ─────────
{
    ...
    check(`Verification: ${verification.checkpointsChecked} checkpoints matched`, verification.verified, verification.firstDivergenceTick !== null ? `divergence at tick ${verification.firstDivergenceTick} (${verification.expectedChecksum} vs ${verification.actualChecksum})` : 'no checkpoints');
    check('Verification: final checksum identical to live match', player.sim.calculateChecksum() === finalChecksum, `${player.sim.calculateChecksum()} vs ${finalChecksum}`);
}
...
// ── Version gate: future format is rejected with a clear error ────────────
{
    ...
    let error = null;
    try {
        decodeReplay(corrupted);
    }
    catch (e) {
        if (e instanceof ReplayFormatError)
            error = e;
    }
...
// ── Tamper detection: modified command log fails checkpoint verification ──
{
    ...
    if (moveFrame) {
        const cmd = moveFrame.commands.find((c) => c.type === CommandType.MOVE);
        ...
        const verification = player.verify();
        check('Tamper detection: modified command log fails verification', !verification.verified, `checked ${verification.checkpointsChecked}`);
    }
    else {
...
// ── Map identity: a replay must re-simulate on its RECORDED map ──────────
{
    ...
    const v2 = p2.verify();
    check('Map identity: replay of a non-default map verifies', v2.verified, v2.firstDivergenceTick !== null ? `divergence at tick ${v2.firstDivergenceTick}` : 'no checkpoints');
...
//# sourceMappingURL=replayV2.test.js.map