import {
  BinReader, BinWriter, decodeChecksum, decodeCommandList, decodeEnvelope, decodeJsonPayload,
...
// ── Envelope rejects garbage ────────────────────────────────────────────
{
  ...
  for (const c of cases) {
    try { decodeEnvelope(c); } catch (e) { if (e instanceof WireError) rejected++; }
  }
...
// ── JSON payload helper ─────────────────────────────────────────────────
{
  ...
  const back = decodeJsonPayload<typeof value>(encodeJsonPayload(value));
...
// ── BinWriter/BinReader bounds ──────────────────────────────────────────
{
  ...
  w.u32(0xffffffff); w.i32(-123456); w.str('привет'); w.u8(7);
  const r = new BinReader(w.finish());
  check('BinWriter growth + mixed round-trip', r.u32() === 0xffffffff && r.i32() === -123456 && r.str() === 'привет' && r.u8() === 7);

  let threw = false;
  try { new BinReader(new Uint8Array(2)).u32(); } catch (e) { threw = e instanceof WireError; }
...
// ── ProtocolChannel: seq/ack/dedup/heartbeat/timeout ────────────────────
{
  ...
  const aReceived: WireKind[] = [];
  const bReceived: WireKind[] = [];
  ...
  for (const f of aOut) b.onData(f);
  ...
  for (const f of bOut) a.onData(f);
