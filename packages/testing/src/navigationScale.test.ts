...
function createSim(seed: number): GameSimulation {
...
function spawnRiflemen(sim: GameSimulation, count: number, baseX: number, baseY: number, cols: number = 16): number[] {
  const ids: number[] = [];
...
// ── Test 2: 100 units converge without eternal jam ──────────────────────────
{
  ...
  for (const id of ids) {
    ...
    if (e && Math.abs(e.x - 50000) < 12000 && Math.abs(e.y - 50000) < 12000) arrivedCount++;
  }
...
// ── Test 3: 500 crossing units — no deadlock, no blocked-tile entry ────────
{
  ...
  for (let t = 0; t < 900; t++) {
    ...
    if (t >= 100 && t % 100 === 0) {
      for (const id of [...groupA, ...groupB]) {
        const e = sim.entities.get(id);
        if (e && !sim.navigation.isWalkableWorld(e.x, e.y)) blockedTileViolations++;
      }
    }
  }
  ...
  for (const id of [...groupA, ...groupB]) {
    ...
    if (e && (Math.abs(e.x - before.x) > 5000 || Math.abs(e.y - before.y) > 5000)) movedCount++;
  }
