...
(0, vitest_1.describe)('Server Admin Command Authorization & Audit Suite', () => {
    (0, vitest_1.it)('should deny non-admin users from executing admin commands', async () => {
        const res = await adminService_1.AdminService.executeCommand({
    ...
    (0, vitest_1.it)('should allow admin role to execute safe global admin commands (help, server-stats)', async () => {
        const resHelp = await adminService_1.AdminService.executeCommand({
            command: 'help',
            args: [],
            userId: 'user-admin-1',
            role: 'admin',
        ...
        const resStats = await adminService_1.AdminService.executeCommand({
            command: 'server-stats',
            args: [],
            userId: 'user-admin-1',
            role: 'admin',
    ...
    (0, vitest_1.it)('should forbid cheat/debug commands in public/ranked matches during production', async () => {
        ...
        const resCheat = await adminService_1.AdminService.executeCommand({
            command: 'give',
            args: ['5000', '0'],
            userId: 'user-admin-1',
            role: 'admin',
